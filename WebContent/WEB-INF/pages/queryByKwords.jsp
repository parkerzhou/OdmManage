...
<body>
	<div
    	...
    	int index=0;
    	%>
			<c:forEach items="${querybykwords}" var="qbk">
				<% index++; %>
				<tr>
					<td width="3%" align="center"><%=index %></td>
					<td width="4%" align="center">${qbk.line.name }</td>
					<td width="3%" align="center">${qbk.client.name }</td>
					<td width="4%" align="center">${qbk.no }</td>
					<td width="4%" align="center">${qbk.pro_name }</td>
					<td width="3%" align="center">${qbk.pro_plan_num }</td>
					<td width="3%" align="center">${qbk.pro_num }</td>
					<td width="4%" align="center">${qbk.bad_ratio }</td>
					<td width="5%"><%=occurtime.get(index-1) %></td>
					<td width="8%">${qbk.body }</td>
					<td width="4%" align="center">${qbk.dept.name }</td>
					<td width="4%" align="center">${qbk.sort.name }</td>
					<td width="9%">${qbk.reason.body }</td>
					<td width="9%">${qbk.solve.tem_metnod }</td>
					<td width="4%" align="center">${qbk.solve.head }</td>
					<td width="9%">${qbk.solve.basic_method }</td>
					<td width="4%"><%=plantime.get(index-1) %></td>
					<td width="4%"><%=closetime.get(index-1) %></td>
					<td width="4%" align="center">${qbk.solve.comfirm }</td>
					<td width="4%" align="center">${qbk.solve.QA_comf }</td>
					<td width="4%"><c:if test="${qbk.state==1}">
							<a
								href="Reply?ques_id=${qbk.id}&first_id=${qbk.first_id}&second_id=${qbk.second_id}"><strong>回复</strong></a>/
    	    <a href="Alter?user_id=${qbk.user_id}&ques_id=${qbk.id}&page=3"><strong>修改</strong></a>
						</c:if></td>
				</tr>
